...
<button class='kifu-viewer-reverse material-icons btn btn-lg btn-outline-secondary'>autorenew</button>
  ...
  loadKeybinds() {
    ...
    this.options.keybinds.addEventListener('keydown', e => {
      switch (e.key) {
        case 'ArrowUp':    this.first(); break;
        case 'ArrowDown':  this.last();  break;
  ...
  loadButtons() {
    ...
    const self = this;
    [...this.options.buttons.getElementsByClassName('kifu-viewer-first')].forEach(e => {
      e.addEventListener('click', function() { self.first(); });
    });
    [...this.options.buttons.getElementsByClassName('kifu-viewer-prev')].forEach(e => {
      e.addEventListener('click', function() { self.prev(); });
    });
    [...this.options.buttons.getElementsByClassName('kifu-viewer-next')].forEach(e => {
      e.addEventListener('click', function() { self.next(); });
    });
    [...this.options.buttons.getElementsByClassName('kifu-viewer-last')].forEach(e => {
      e.addEventListener('click', function() { self.last(); });
    });
    [...this.options.buttons.getElementsByClassName('kifu-viewer-reverse')].forEach(e => {
      e.addEventListener('click', function() { self.reverse(); });
    });
  }
  ...
  static オブジェクトコピー(from) {
    ...
    for (const key in from) {
      to[key] = (from[key] instanceof Object)  ?  KifuViewer.オブジェクトコピー(from[key])  :  from[key];
    }
    return to;
  }
  ...
  static svgText(x, y, fontSize, text, textAnchor) {
    ...
    node.setAttribute('font-family', 'YuMincho, Hiragino Mincho ProN, MS Mincho, serif');
    ...
    if (前指し手 && Math.abs(指し手.手数 - 前指し手.手数) == 1) {  // 差分描画 prev/next/first/last
      if (指し手.手数 < 前指し手.手数) {  // prev/first
        ...
        if (前指し手.前X != 0) {  // 駒台からの着手以外
          this.駒描画(redraw, pieces, 局面.駒[前指し手.前Y][前指し手.前X], 前指し手.前X, 前指し手.前Y, false, 反転);
        }
        this.駒描画(redraw, pieces, 局面.駒[前指し手.後Y][前指し手.後X], 前指し手.後X, 前指し手.後Y, false, 反転);
      } else {  // next/last
      ...
      if (指し手.手数 == 0) {
        ...
        if (指し手.前X != 0) {  // 駒台からの着手以外
          this.駒描画(redraw, pieces, 局面.駒[指し手.前Y][指し手.前X], 指し手.前X, 指し手.前Y, false, 反転);
        }
        this.駒描画(redraw, pieces, 局面.駒[指し手.後Y][指し手.後X], 指し手.後X, 指し手.後Y, true, 反転);
      }
  ...
  駒描画(redraw, pieces, 駒, x, y, 最終手, 反転) {
    ...
    if (駒.match('_')) { //後手
      piece.setAttribute('transform', `translate(${筋}, ${段 + 8}) scale(-1, -1)`);
    } else { //先手
  ...
  上下名称表示(redraw, 反転) {
    ...
    if (redraw) {
      const g_childNodes = this.board.childNodes[7].childNodes;
      g_childNodes[0].textContent = 上名称;
      g_childNodes[1].textContent = 下名称;
      g_childNodes[0].setAttribute('font-size', 上サイズ);
      g_childNodes[1].setAttribute('font-size', 下サイズ);
    } else {
  ...
  枡目表示() {
    ...
    for (let i = 1; i <= 8; i++) {
      fragment.appendChild(KifuViewer.svgLine(12 + 40 * i, 77.5, 12 + 40 * i, 360 + 77.5));
      fragment.appendChild(KifuViewer.svgLine(12, 77.5 + 40 * i, 370, 77.5 + 40 * i));
    }
    return fragment;
  }

  符号表示(redraw, 反転) {
    let text_n = new Array("１", "２", "３", "４", "５", "６", "７", "８", "９");
    let text_k = new Array("一", "二", "三", "四", "五", "六", "七", "八", "九");
    if (反転) {
      text_n = new Array("９", "８", "７", "６", "５", "４", "３", "２", "１");
      text_k = new Array("九", "八", "七", "六", "五", "四", "三", "二", "一");
    }
    if (redraw) {
      const g_childNodes = this.board.childNodes[6].childNodes;
      for (let i = 0; i <= 8; i+=1) {
        g_childNodes[i * 2].textContent = text_n[i];
        g_childNodes[i * 2 + 1].textContent = text_k[i];
      }
    } else {
      const g = KifuViewer.svg('g');
      this.board.appendChild(g);
      for (let i = 0; i <= 8; i++) {
        g.appendChild(KifuViewer.svgText(355 - 40 * i, 73, 14, text_n[i], 'middle'));
        g.appendChild(KifuViewer.svgText(386, 100 + 40 * i, 14, text_k[i], 'middle'));
      }
    }
  }
...
KifuViewer.全局面構築.各局面 = function(指し手, 前局面) {
  ...
  let 局面 = KifuViewer.オブジェクトコピー(前局面);
...
KifuViewer.KIF解析.手合割 = function(kif手合割) {
  const 手合割 = ["香落ち", "右香落ち", "角落ち", "飛車落ち", "飛香落ち", "二枚落ち", "三枚落ち",
    "四枚落ち", "五枚落ち", "左五枚落ち", "六枚落ち", "八枚落ち", "十枚落ち", "その他"];
...
KifuViewer.KIF解析.局面図 = function(kif局面図, 手合割) {
  ...
  let 局面 = KifuViewer.KIF解析.局面図.駒無し();
  let 先手 = true;
  let x    = 10;
  const 変換 = {'王':'玉', '竜':'龍'};
...
KifuViewer.KIF解析.局面図.手合割 = function(手合割) {
  let 局面 = KifuViewer.KIF解析.局面図.平手();
  switch (手合割) {
    case '香落ち':   局面[1][1] = null;  return 局面;
    case '右香落ち': 局面[1][9] = null;  return 局面;
    case '角落ち':   局面[2][2] = null;  return 局面;
    case '飛車落ち': 局面[2][8] = null;  return 局面;
    case '飛車香落ち':
      局面[1][1] = null;
      局面[2][8] = null;
      return 局面;
...
KifuViewer.KIF解析.持駒 = function(kif持駒) {
  let 持駒 = {'歩': 0, '香': 0, '桂': 0, '銀': 0, '金': 0, '飛': 0, '角': 0};
  if (kif持駒 === undefined || kif持駒.match('なし')) {
    return 持駒;
  }
  const 漢数字 = {'一':1, '二':2, '三':3, '四':4, '五':5, '六':6, '七':7, '八':8, '九':9, '十':10,
...
KifuViewer.KIF解析.指し手.現在の手 = function(全指し手, kif, 手数, 開始手番) {
  ...
  const 解析 = 現在の手.match(/([１-９同])([一二三四五六七八九　])([^\(]+)(\((\d)(\d)\))?/);
    ...
    if (解析) {
      ...
      全指し手.push({'手数':手数, '手番':手番, '手':現在の手, '駒':'終', '前X':0, '前Y':0,
        '後X':前指し手.後X, '後Y':前指し手.後Y, '成り':false, 'コメント':''});
      全指し手.勝敗 = KifuViewer.KIF解析.指し手.勝敗(現在の手, 手番);
    }
};

KifuViewer.KIF解析.指し手.勝敗 = function (理由, 手番) {
  let 結果 = {'勝者':'', '敗者':'', '理由':理由, '表記':''};
  switch (理由) {
    case '投了': case '詰み': case '切れ負け': case '反則負け':
      結果.勝者 = (手番 === '▲') ? '△' : '▲';
      結果.敗者 = (手番 === '▲') ? '▲' : '△';
